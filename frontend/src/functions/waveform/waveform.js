...
import Cursor from "../wavesurferCursor/index";
import Markers from "../wavesurferMarkers/index";
import { api } from "../../composables/custom";
import { trackList } from "../../stores/globalStores";


export var wavesurfer = [];
export function createWavesurfer(audio, id, setwaveColor, setprogressColor, isSplitChannels, waveformHeight) {
...
const updateCursorPosition = (id) => {
  ...
  if (liveCursorPiano) {
    ...
    pianoroll.getElementsByTagName("div")[2].scrollLeft =
      currentTrackList.selectTrack(id).pianoroll.scrollLeft + wavesurfer[id].drawer.wrapper.scrollLeft;
  }
};
export function trimWaveform(trackname, start, end, selectedTrackIndex, fromBar, toBar) {
  ...
  for (var i = 0; i < originalBuffer.numberOfChannels; i++) {
    ...
    for (
      var j = 0, f = Math.round(start * originalBuffer.sampleRate), t = Math.round(0 * originalBuffer.sampleRate), len = chanData.length;
      j < len;
      j++, f++, t++
    ) {
  ...
  api.get("/trim-audio/" + trackname + "/" + start + "/" + end + "/" + fromBar + "/" + toBar).then((response) => {
