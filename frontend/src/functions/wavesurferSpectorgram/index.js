import FFT from "../wavesurferSpectorgram/fft";
...
export default class SpectrogramPlugin {
  ...
  createWrapper() {
    ...
    setTimeout(() => {
      ...
      this.loadLegend("rgba(68,68,68,0.5)", "12px", "10px", "", "#fff", "#f7f7f7", "center", "#specLabels");
    }, 0);
  ...
  loadLegend(bgFill, fontSizeFreq, fontSizeUnit, fontType, textColorFreq, textColorUnit, textAlign, container) {
  ...
  drawColorMaps(ctx, marginColorbar) {
    ...
    for (var j = 0; j < 256; j++) {
      ctx.fillStyle = `rgba(${this.colorMap[j][0] * 255}, ${this.colorMap[j][1] * 255}, ${this.colorMap[j][2] * 255}, ${
        this.colorMap[j][3] * 255
      })`; // start ind at index 0
