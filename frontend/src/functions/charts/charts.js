...
export default class ChartManager {
  constructor(data, id, type, graph_color, graph_type, duration) {
    ...
    this.chart = new Chart(document.getElementById(`chart-${type}-${id}`), {
      type: graph_type || "bar",
      data: {
        labels: this.labels,
        datasets: [
  ...
  createHistogramLength(numBins) {
    ...
    this.chart.options.plugins.title.text = `Length of Intervals`;
  ...
  createHistogramTime(numBins) {
    ...
    for (let i = 0; i < numBins; i++) {
      ...
      const binLabel = `${startMinutes.toString().padStart(2, "0")}:${startSeconds.toString().padStart(2, "0")} - ${endMinutes
        .toString()
    ...
    (this.chart.data.datasets.backgroundColor = this.graph_color), (this.chart.options.scales.x.title.text = `Time Interval [min:sec]`);
    this.chart.options.scales.y.title.text = `Interval Count`;
    this.chart.options.plugins.title.text = `The number of ${this.type} in parts of a musical recording`;
    this.chart.update();
  }
}
