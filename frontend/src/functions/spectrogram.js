...
import { createWavesurfer } from './waveform'
import {wavesurfer} from './waveform'


export const createSpectrogram = async (trackName) => {
    const my = {
        height: 256,
        width: 1000,
        freqFrom: 5000,
...
const config = {
    ...
    processorBufferSize: 2048,
        ...
        const loadWaveForm = async (trackName) => {
            // // Convert fetch to array buffer
            const resp = await fetch('/9904__snoman__grass1.wav')
            // let source
        ...
        const processAudio = async (audioBuffer) => {
            ...
            processor.onaudioprocess = (ev) => {
                ...
                for (let i = 0; i < source.channelCount; i += 1) {
                    ...
                    analyzers[i].getByteTimeDomainData(freqData)
                }
                offset += generalAnalyzer.frequencyBinCount
            }
            // frequencyBinCount - half that of the AnalyserNode.fftSize.
        ...
        const run = async() => {
            const audioBuffer = await loadWaveForm (trackName)
            const processed = await processAudio(audioBuffer)
            const spectrogram = drawSpectrogram(processed.channels)
            ...
            // for (let i = 0; i < processed.channels[0].length; i ++) {
                
            //     let rat = processed.channels[0][i]
            //     canvasCtx.beginPath()
            //     canvasCtx.strokeStyle = `rgba(0,0,0,${rat})`
            //     canvasCtx.lineWidth = 15
            //     canvasCtx.moveTo(1,1)
            //     canvasCtx.lineTo(1,3)
            //     canvasCtx.stroke()
                
            // }
            // console.log(processed.channels[0].length);
...
// export const createSpectrogram = (trackName) => {
...
//     const resp = await fetch('/chopin_07.wav')
//     resp.crossOrigin = ""
//     const waveDataBuffer = await resp.arrayBuffer()
...
//         const waveform = await loadWaveForm(trackName)
