...
import UploadModal from "../components/uploadManager/UploadModal.vue";
...
<template>
  ...
  <div class="min-h-full">
    ...
    <div id="main-container" class="h-[calc(100vh-4.5rem)] flex overflow-hidden relative">
      <div
        id="tracklist"
        class="w-62 h-[calc(100vh-12.5rem)]"
        :class="{ 'transform -translate-x-[97%] border border-black': hideTracklistPanel }"
      >
        <div id="tracklist-divider" class="w-[97%] h-full bg-white">
          <div id="tracklist-content" class="w-full h-full pb-5 flex flex-col items-center gap-1 overflow-y-auto">
        ...
        <div id="tracklist-button" class="w-[3%] h-full">
          <button
            @click="hideTracklistPanel = !hideTracklistPanel"
            id="arrow"
            class="h-50 border-black"
            :class="{ ' border-l-[0.5rem] border-r-[0rem]': hideTracklistPanel, 'border-r-[0.5rem] border-l-[0rem]': !hideTracklistPanel }"
          ></button>
        </div>
      </div>
      ...
      >
        <div id="tracklist-button" class="w-[7%] h-full">
          <button
            @click="hideSettingPanel = !hideSettingPanel"
            id="arrow-left"
            class="h-50 border-black"
            :class="{ ' border-r-[0.5rem] border-l-[0rem]': hideSettingPanel, 'border-l-[0.5rem] border-r-[0rem]': !hideSettingPanel }"
          ></button>
        </div>
        <div v-for="track in currentTrackList.trackState" :key="track.id">
          <RightPanel
