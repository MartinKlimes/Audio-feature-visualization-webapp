...
const upload = ($event) => {
  ...
  for (let i = 0; i < files.length; i++) {
    if (
      (files[i].type.match(audioTypes) && !currentTrackList.trackState.includes(files[i].name)) ||
          ...
          <div class="bg-white rounded border border-gray-200 relative flex flex-col">
            ...
            <div class="p-6">
              ...
              <div class="h-auto max-h-72 overflow-y-auto">
                <div v-for="(filename, i) in fileNameList" :id="`file-${i}`" :key="filename">
                  <div class="mb-4">
                    <!-- File Name -->
                    <div class="font-bold text-sm" :class="{ 'text-green-600': isUploaded[i] }">
                      <Icon icon="fa:spinner" class="spin" :class="{ hidden: isUploaded[i] }" />
                      {{ filename }}
                      <Icon icon="fa:check" :class="{ hidden: !isUploaded[i] }" />
                    </div>
                    <div class="flex h-4 overflow-hidden bg-gray-200 rounded">
                      <!-- Inner Progress Bar -->
                      <div :id="`progress-bar-${i}`" class="transition-all progress-bar bg-blue-400"></div>
                    </div>
                  </div>
                </div>
              </div>
