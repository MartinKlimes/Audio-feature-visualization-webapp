...
const deleteVisualizations = (visualization, isVisualization,visualizationType) => {
  if(trackToEdit.value[visualization][isVisualization]){
    trackToEdit.value[visualization][isVisualization] = false;
    trackToEdit.value[visualization][isVisualization + 'Displayed'] = false;
    updateRecording(trackToEdit.value.id, isVisualization, false, visualizationType);
    updateRecording(trackToEdit.value.id, isVisualization + 'Displayed', false, visualizationType);
  }
}
const useTrack = () => {
  ...
  if (trackToEdit.value.isTrackSelected == false) {
    ...
    deleteVisualizations('waveform', 'isWaveform')
    deleteVisualizations('spectrogram', 'isSpectrogram')
    deleteVisualizations('pianoroll', 'isPianoroll')
    deleteVisualizations('ioi_data', 'isIOI', 'ioi_data')
    deleteVisualizations('ibi_data', 'isIBI', 'ibi_data')
    deleteVisualizations('imi_data', 'isIMI', 'imi_data')
    deleteVisualizations('Tempo_data', 'isTempo', 'Tempo_data')
    deleteVisualizations('RMS_data', 'isRMS', 'RMS_data')
...
<template>
  ...
  >
    ...
    <div v-show="state.showForm">
      <div v-if="state.showAlert" class="text-white text-center font-bold p-4 mb-4" :class="state.alert_variant">
        {{ state.alert_message }}
      </div>
      ...
      <form>
        ...
        <div v-else>
          <button type="submit" class="py-1 px-2 rounded text-white bg-gray-600" :disabled="v$.modifiedName.$invalid">Submit</button>
        </div>
      </form>
    </div>
  </div>
