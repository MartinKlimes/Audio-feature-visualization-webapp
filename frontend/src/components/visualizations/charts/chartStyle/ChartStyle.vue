...
<template>
  <div class="flex flex-col bg-white box rounded-md py-2 px-1 w-full items-center gap-1">
    <div class="flex gap-1">
      ...
      <BlueButtons
        :icon="
          currentTrackList.selectTrack(props.id)[props.selectedIntervalVis].graph_type == 'line'
    ...
    <Transition>
      <ColorsPicker
        ...
        :selected-color="currentTrackList.selectTrack(props.id)[props.selectedIntervalVis].graph_color"
        class="h-27 w-full"
        @update-color="changeColor($event)"
        :format="'hex'"
      />
    </Transition>
  </div>
