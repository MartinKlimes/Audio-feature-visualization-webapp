...
import ChartAverage from "./ChartAverage.vue";
import ChartStyle from "../chartStyle/ChartStyle.vue";
import ChartHistograms from "./ChartHistograms.vue";
...
<template>
  <div class="flex flex-col items-center relative h-max w-full p-2 bg-gray-200 rounded-md border border-gray-300 shadow-md gap-2">
    <ExportBtn
      ...
      :visualization="
        selectedVis == 3 ? 'measure' : selectedVis == 4 ? 'onset' : selectedVis == 5 ? 'beats' : selectedVis == 6 ? 'Tempo' : 'RMS'
      "
    ...
    <Transition>
      <ChartStyle v-if="showIntervalStyle" :id="track.id" :selectedIntervalVis="selectedIntervalVis" />
    </Transition>

    <Transition>
      <ChartAverage v-if="showAverageSetting" :id="track.id" :selectedIntervalVis="selectedIntervalVis" />
    </Transition>

    <Transition>
      <ChartHistograms v-if="showHistogramSetting" :id="track.id" :selectedIntervalVis="selectedIntervalVis" />
    </Transition>
