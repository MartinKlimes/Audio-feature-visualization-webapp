...
const toggleHorizontalAverage = () => {
  currentTrackList.selectTrack(props.id)[props.selectedIntervalVis].showHorizontalAverage = !currentTrackList.selectTrack(props.id)[
    props.selectedIntervalVis
  ].showHorizontalAverage;
};
const toggleMovingAverage = () => {
  currentTrackList.selectTrack(props.id)[props.selectedIntervalVis].showMovingAverage = !currentTrackList.selectTrack(props.id)[
    props.selectedIntervalVis
  ].showMovingAverage;
};
</script>
  ...
  <div class="boxTight mt-2 w-full h-max bg-gray-100 rounded-md flex items-start pt-1 flex-col justify-center">
    <div>
      <input
        ...
        type="checkbox"
        @change="toggleHorizontalAverage"
