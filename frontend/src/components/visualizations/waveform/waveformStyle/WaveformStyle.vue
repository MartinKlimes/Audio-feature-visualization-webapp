<script setup>
import ColorWaveform from "../waveformStyle/ColorWaveform.vue";
...
<template>
  <div class="flex flex-col bg-white box rounded-md p-2 mt-1 w-full items-center" ref="target">
    ...
    <Transition>
      <ColorWaveform v-if="showColorPicker" :id="id" :trackName="trackName" :waveformColor="waveformColor" />
    </Transition>

    <Transition>
      <SizeSetter v-if="showSizeSetter" :id="id" :visualization="'waveform'" :loading-visualization="'isWaveformLoading'" />
    </Transition>
  </div>
</template>
