...
<template>
  ...
  <div justify="between" class="rounded-sm py-0.5" :class="track.backgroundColor">
    <div class="relative mt-1" :id="`waveformContainer-${track.id}`" :class="[{ 'shadow-md shadow-gray-500': props.isSelected }]">
      <transition>
        <div :id="`waveform-${track.id}`">
          ...
          <transition>
            <div
              v-show="showZoomerSetting"
              class="absolute right-3 gap-1 bottom-10 bg-gray-400 rounded-md flex z-10 z-10 cursor-pointer"
              data-html2canvas-ignore="true"
            >
              <Icon icon="zondicons:refresh" class="hover:bg-white hover:border hover:border-black rounded-l-md" @click="refreshZoomer" />
              <Icon
                :icon="hideZoomer ? 'mdi:show' : 'mdi:hide'"
                class="hover:bg-white hover:border hover:border-black rounded-r-md"
                @click="hideZoomer = !hideZoomer"
              />
            </div>
          </transition>
