...
import ClickSoundBtn from "../../../MainPanel/ClickSoundBtn.vue";
    ...
    () => {
      if (document.querySelector(markerID.value)) {
        ...
        if (props.dataType == "bars") {
          ...
          api
            .get("/get-event-detection/" + props.trackName, {
...
<template>
  <div class="flex flex-col items-center gap-1 mt-1 w-full">
    <div class="grid grid-cols-2 gap-1 boxTight w-21 rounded-md p-2">
      ...
      <BlueButtons :icon="'ph:list'" :icon-class="'ml-0 '" @click="changeLine = !changeLine" :is-btn-clicked="changeLine"></BlueButtons>
