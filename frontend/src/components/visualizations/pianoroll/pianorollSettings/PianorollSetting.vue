...
const dynamicNames = () => {
  currentTrackList.selectTrack(props.track.id).pianoroll.dynamicNames = !currentTrackList.selectTrack(props.track.id).pianoroll
    .dynamicNames;
...
<template>
  <div class="flex flex-col items-center relative h-max w-full p-2 bg-gray-200 rounded-md border border-gray-300 shadow-md">
    ...
    <div
          class=" z-1 rounded-md text-xs px-1 py-0.5 shadow-inner shadow-gray-500" 
    ...
    <Transition>
      <div v-if="showColorPicker">
        <ColorPicker
          @color-change="changeColor($event.cssColor)"
          default-format="hex"
